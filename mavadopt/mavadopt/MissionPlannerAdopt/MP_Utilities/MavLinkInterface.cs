...
namespace ITMS.Logic.MP_Utilities
{
    public class MAVLinkInterface: MAVLink, IDisposable
    {
        ...
        {
            OpenBg(true);//sender, true, e);
        }

        //TUTUP - ADAM
        void FrmProgressReporterDoWorkNOParams()//object sender, ProgressWorkerEventArgs e, object passdata = null)
        {
            OpenBg(false);//sender, false, e);
        }

         //TUTUP - ADAM
        private void OpenBg(bool getparams)//object PRsender, bool getparams, ProgressWorkerEventArgs progressWorkerEventArgs)
        {
            //frmProgressReporter.UpdateProgressAndStatus(-1, Strings.MavlinkConnecting);
        ...
        4. Try a diffrent usb port\n\n" + "No Mavlink Heartbeat Packets where read from this port - Verify Baud Rate and setup\nMission Planner waits for 2 valid heartbeat packets before connecting");
            ...
            MessageBox.Show("Now Getting Param!");//Strings.Done);
        ...
        */
        public void getParamList()
        {
            ...
            foreach (string item in MAV.param.Keys)
            {
                if (float.IsNaN((float)MAV.param[item]))
                    MessageBox.Show("BAD PARAM, " + item + " = NAN \n Fix this NOW!!", Strings.ERROR);
            }
        }
        ...
        {
            //int planforremoval;//?? g ada yg pake
        ...
        public bool doMotorTest(int motor, MAVLink.MOTOR_TEST_THROTTLE_TYPE thr_type, int throttle, int timeout, MAVLinkInterface _mavreference)
        {
            return _mavreference.doCommand(MAVLink.MAV_CMD.DO_MOTOR_TEST, (float)motor, (float)(byte)thr_type, (float)throttle, (float)timeout, 0, 0, 0);
        }
        ...
        public byte[] readPacket()
        {
            ...
            try
            {
                if ((buffer[0] == 'U' || buffer[0] == 254) && buffer.Length >= buffer[1])
                {
                    ...
                    lock (objlock)
                    {
                        //tulis pake buat CS, sesuai id message
        ...
        public void setAPType(byte sysid)
        {
            ...
            switch (MAVlist[sysid].apname)
            {
                case MAV_AUTOPILOT.ARDUPILOTMEGA:
                    switch (MAVlist[sysid].aptype)
                    {
                        case MAVLink.MAV_TYPE.FIXED_WING:
                            MAVlist[sysid].cs.firmware = Firmwares.ArduPlane;
                            break;
                        case MAVLink.MAV_TYPE.QUADROTOR:
                            MAVlist[sysid].cs.firmware = Firmwares.ArduCopter2;
                            break;
                        ...
                        case MAVLink.MAV_TYPE.GROUND_ROVER:
                            MAVlist[sysid].cs.firmware = Firmwares.ArduRover;
                            break;
                        case MAV_TYPE.ANTENNA_TRACKER:
                            MAVlist[sysid].cs.firmware = Firmwares.ArduTracker;
                            break;
                        default:
                            break;
                    }
                ...
                case MAV_AUTOPILOT.GENERIC:
                    ...
                    {
                        case MAVLink.MAV_TYPE.FIXED_WING:
                            MAVlist[sysid].cs.firmware = Firmwares.Ateryx;
                            break;
                    }
                    break;
            }
            ...
            switch (MAVlist[sysid].cs.firmware)
            {
                case Firmwares.ArduCopter2:
                    MAVlist[sysid].Guid = Form1.config["copter_guid"].ToString();//P_0601_Setup.config["copter_guid"].ToString();
                    break;
                case Firmwares.ArduPlane:
                    MAVlist[sysid].Guid = Form1.config["plane_guid"].ToString();
                    break;
                case Firmwares.ArduRover:
                    MAVlist[sysid].Guid = Form1.config["rover_guid"].ToString();
                    break;
            }
        }
